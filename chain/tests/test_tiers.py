"""The three-phase epoch: partitions, standing, and the top-tier mempool."""
import dataclasses
...
def test_an_epoch_reaches_the_top_tier_mempool():
    w, wallets, tx = funded()
...
def test_every_tier_runs_a_ceremony():
    ...
    assert len(r.tier0.ceremonies) == len(w.topology.grid_ids())
    assert all(c.finalised for c in r.tier0.ceremonies.values())
    assert r.tier1.ceremonies and all(c.finalised for c in r.tier1.ceremonies.values())
    assert r.tier2 is not None and r.tier2.finalised
...
def test_only_the_owning_grid_includes_it():
    w, wallets, tx = funded()
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
    carrying = [gid for gid, b in r.tier0.blocks.items() if b.transactions]
...
def test_apprentice_attestations_do_not_count_toward_quorum():
    ...
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
    for gid, res in r.tier0.ceremonies.items():
        reg = w.registers[gid]
...
def test_apprentices_still_appear_in_the_attendance_roll():
    ...
    roll = r.tier0.ceremonies[gid].roll
...
def _reject(w, epoch, mutate):
    ...
    cer = Ceremony(grid, {n: w.nodes[n] for n in members}, PARAMS,
                   ...
                   workload=GridWorkload(w, gids[0], epoch,
...
def test_scrambled_trust_lists_do_not_disturb_an_epoch():
    ...
    result = run_tiered_epoch(w, epoch=1, base_seed="s")
    assert result.finalised, result.reason
    assert len(result.tier0.finalised) == len(w.topology.grid_ids())
    assert sum(1 for _ in result.block.transactions()) == 1
...
def test_the_top_tier_collapses_when_there_is_one_tier1_grid():
    w, wallets, tx = funded(n=10)          # 2 grids -> 2 leaders -> 1 tier-1 grid
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
    assert r.finalised, r.reason
    assert r.tiers == 2, "one tier-1 grid should collapse the top tier onto it"


def test_three_tiers_when_the_roster_is_large_enough():
    w, wallets, tx = funded(n=40)          # 8 grids -> 2 tier-1 grids -> the top tier
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
...
def test_the_top_tier_seats_the_tier_below_and_not_its_leaders():
    """Review C2. One seat per tier-1 grid made the whole network's liveness as
    fragile as one tiny grid's — and the tiny grid was the one every partition
    depended on."""
    w, _, _ = funded(n=40)
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
    assert r.finalised, r.reason
    seated = set(r.tier2.grid.seats)
    below = {n for res in r.tier1.finalised.values() for n in res.grid.seats}
    assert seated == below, "the committee is every seat of every tier-1 grid"
    leaders = {res.grid.leader for res in r.tier1.finalised.values()}
...
def test_a_committee_of_leaders_tolerated_no_absence_at_all():
    ...
    At this size the old rule seated one node per tier-1 grid: two seats, quorum
    ...
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
    assert r.finalised, r.reason
    seats = len(r.tier2.grid.seats)
    tolerated = seats - r.block.header.quorum
    old_seats = len(r.tier1.finalised)
...
def test_one_tier1_grid_is_the_same_rule_and_not_a_special_case():
    """The collapse to two tiers used to be a separate branch seating the only
    tier-1 grid's members.  That is now what the general rule already does."""
    w, _, _ = funded(n=10)                 # 2 grids -> 2 leaders -> 1 tier-1 grid
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
    assert r.finalised and r.tiers == 2
    only = next(iter(r.tier1.finalised.values()))
    assert set(r.tier2.grid.seats) == set(only.grid.seats)


def test_a_member_may_ask_whether_its_own_tier1_grid_was_left_out():
    """`owner_of` answers "was my grid omitted", and a member that is not a
    leader has exactly as much right to ask."""
    import dataclasses as _dc

    from ..tiers import RootWorkload

    w, _, _ = funded(n=40)
    r = run_tiered_epoch(w, epoch=1, base_seed="s")
    assert r.finalised, r.reason
    block = r.block
    dropped_id = block.groups[0].header.group_id
    thinned = _dc.replace(block, groups=block.groups[1:])
    # Re-root it, so the block is not refused for the shallower reason first.
    thinned = _dc.replace(thinned, header=_dc.replace(
        block.header, group_root=thinned.compute_group_root()))
    owner = {nid: sid for sid, res in r.tier1.finalised.items()
             for nid in res.grid.seats}
    mine = [n for n, sid in owner.items()
            if sid == dropped_id
            and n != r.tier1.finalised[dropped_id].grid.leader]
    assert mine, "this fixture needs a tier-1 grid with a non-leader member"
    work = RootWorkload(w, r.tier1.blocks, 1, owner, tiers=r.tiers,
...
def test_the_single_grid_case_collapses_to_one_tier():
    ...
    assert len(block.groups) == 1 and len(block.groups[0].children) == 1
    assert block.quorum_cert is not None, "the network block is what was agreed"
    assert block.groups[0].quorum_cert is None
    assert block.groups[0].children[0].quorum_cert is None
