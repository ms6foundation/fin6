...
```
Phase L   tier-0 grids, in parallel        →  CeremonyBlock  →  tier-0 mempool
Phase S   their leaders form tier-1 grids  →  GroupBlock     →  tier-1 mempool
Phase X   tier-1 leaders → top tier    →  NetworkBlock   →  top-tier mempool
```
...
the top-tier mempool agreed but reversible, and enters history when turns from a
...
top-tier mempool, grid registers and standing, nullifier partitioning, and
...
- [`docs/supreme_tier_design.md`](docs/supreme_tier_design.md) — twelve: the top tier as a global stall point, and the roads out of it *(partly built)*
...
- [`docs/tier_naming_decision.md`](docs/tier_naming_decision.md) — why local, super and supreme were inside the chain id, and what replaces them *(applied)*
