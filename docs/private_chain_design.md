...
| Quorum signature scheme | **The shape is now chosen; the scheme is not.** A certificate was a list of whole attestations, each restating the chain id, height, block hash, epoch and grid seed the certificate states once, and carrying a key the roster already holds. It is now `signers` plus `signatures` checked against the roster — measured at 84 bytes a seat against 137, so 1.6× on the wire (less than it looks, because `codec` was already interning the repeated fields) and 54.7 KB for a 667-seat quorum. The size is not the point: signatures are now 64 of those 84 bytes, so an aggregate scheme collapses the list to one signature and leaves everything else untouched — 6.8 KB, and about 0.2 KB if `signers` also becomes a true bitmap over a canonical seat order, which needs that order committed in the header. Choosing **threshold BLS or another scheme is a dependency decision**: the repo has one runtime dependency and pairings are not in it. |
