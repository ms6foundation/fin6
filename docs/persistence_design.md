...
| Nullifiers never shrink | Still true — 27 MB/day at 10 tx/s — and chasing *whether the set is load-bearing* mattered more than bounding it. Two findings.

**The set was a liability, and part nine fixed that.** The nullifier was one public quadratic form over the note's coordinates — eight at DEMO, forty-eight at STRONG — mapping onto a single field element. Homogeneous of degree 2, so Q(-x) = Q(x), and solving Q(x) = t for one unconstrained blinder is one square root mod P. A payer chooses every coordinate of the note it hands you, so it could craft your note to share a nullifier with one it held, spend its own, and leave yours live in the UTXO set and refused by every node as a double spend. One payment to freeze a stranger's funds. `nullifier_id` now binds `cm`, which makes the published marker as discriminating as the commitment; `chain/tests/test_nullifiers.py` constructs the old collision and shows the new id does not.

**And the check is redundant, which is why removing it was ever thinkable.** This is the confidential-transactions model, not the Zcash one: the spend graph is public, so a transaction names the note it spends. `SealAccumulator.__contains__` already returns false for a spent position, so `tin.cm not in self.utxo` refuses a replay before the nullifier check is reached. A nullifier set exists in Zcash *because* you cannot reveal the note. Here you do. So the class-A term could be scoped or dropped outright with no soundness argument about double spending — what it would break is the wallet protocol, which learns a note was spent by watching for its nullifier in `getoutputs`, and `nf_root` in every header. Bounded by expiry, dropped, or kept as a wallet index: all three are now open choices rather than a cost with no options, and none of them is urgent, because the acute problem was never the storage. |
| Snapshot cadence | Export and import work, and part nine gave them a *use*: a node past the body window asks a peer for state, which is exported on demand rather than from a schedule. So the open question narrows — *stored* snapshots still have no cadence, and on-demand export costs the serving node a full state copy at the moment it is asked, which nothing rate-limits beyond the peer budget. |
