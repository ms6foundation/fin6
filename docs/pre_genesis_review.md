...
| C2 | ~~**The supreme grid is a global stall point**~~ **Done** | High | If it aborts, nothing finalises anywhere that epoch. Four of part twelve's five items are built: the committee is the whole tier below rather than its leaders (2 seats and *zero* tolerated absences became 8 and two, in the test fixture), the supreme tier changes view instead of losing the epoch, service at tiers 1 and 2 is credited in each seat's home register, and activation slots are reserved — the only part that expired at genesis. Road B is left, with its reason, in §9.5. Sketched as part twelve — `docs/supreme_tier_design.md`: the committee is seven seats at 343 nodes, so three absences stall the network; four roads, of which only reserving activation heights expires at genesis. |
