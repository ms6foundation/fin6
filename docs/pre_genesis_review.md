...
| C2 | **The supreme grid is a global stall point** | High | If it aborts, nothing finalises anywhere that epoch. Sketched as part twelve — `docs/supreme_tier_design.md`: the committee is seven seats at 343 nodes, so three absences stall the network; four roads, of which only reserving activation heights expires at genesis. |
