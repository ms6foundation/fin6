...
The review's line is four words long — *the top tier is a global stall
point* — and reading the code to size the work turned up two things the line
does not say.

The first is a number. The top tier is seated from the *leaders* of the
tier-1 grids, so it has one seat per tier-1 grid: at the sizing rule's own
...
The second is that **the top tier has never run over a network**.
...
|---|---|---|
| local | skipped; its transactions stay in mempools and wait for the next epoch | `"no tier-0 grid finalised"` — the epoch aborts |
| super | its children's blocks are dropped for this epoch — `g` grids' work, silently | `"no tier-1 grid finalised"` — the epoch aborts |
...
top. A tier-0 grid that cannot reach quorum costs its own partition one epoch.
The top tier that cannot reach quorum costs *every* partition one epoch.
...
where a real network lives. And the top-tier seats are not a random sample: they
...
`sorted(self.supers)`. So the top tier is not choosing a block. **The only
free variable is which certified tier-1 blocks are in the set**, and two honest
...
The top tier seats **the leaders of the tier-1 grids**. Seat **the super
grids' members** instead.

At 343 nodes that is 49 seats rather than 7 — one per tier-0 grid — and it
wakes nobody new: those nodes are already awake, already in this epoch's
ceremony, already holding the tier-1 blocks. It is the same change at two tiers
...
1. A proposal carries its tier-1 blocks with their certificates, so a seat can
   verify a tier-1 block it has never seen — inclusion is checkable, not
   trusted.
...
The top tier has no register. Standing lives in `GridRegister`, one per
*local* grid, advanced by an `AttendanceRoll` carried in the block — and the
top tier is not a tier-0 grid, has no persistent membership, and produces no
roll of its own. So a node that no-shows at tier 1 or tier 2 pays nothing,
while the same node missing its tier-0 ceremony loses its attendance streak.
...
- whether a missed top-tier ceremony should reset a counter or merely fail to
...
| | work | fixes | cost |
|---|---|---|---|
| 1 | ~~**Road A** — seat the tier-1 grids' members, not their leaders~~ **Done** | the committee, which is where §2's numbers live | one selection rule; latency and certificate size |
...
| 3 | ~~**Road C** — C1's views at the top tier~~ **Done in process** | the dead leader | budget arithmetic; reuses `viewchange.py`. See §9.4 |
...
`supreme_members` is the union of the seats of every tier-1 grid that finalised,
...
branch that seated the only tier-1 grid's members; the union of one tier-1 grid's
seats *is* that grid, so the branch is gone and both cases are one rule.

**`owner_of` had to widen with it.** It answers "was my own tier-1 grid left out
...
eight tier-0 grids, two tier-1 grids:

| | seats | quorum | absences tolerated |
|---|---|---|---|
| one leader per tier-1 grid | 2 | 2 | **0** |
...
what makes it checkable rather than announced. A tier-1 grid's seats are the
leaders of the children it carries; the top-tier committee is the union of
...
`GridWorkload._register_after` applies it in exactly the order
...
*Does a missed top-tier ceremony reset a local counter?* **No**, and the
...
The supreme phase now runs up to `TOP_VIEWS` views. Each reseats the whole
...
there was one tier: silencing the top-tier leader also silenced it in its own
...
a proposal carrying tier-1 blocks a seat does not hold is a larger set, and it
...
- **It does not make the top tier optional.** Global roots need a global
...
- **It does not address the tier 1's silent loss.** A tier-1 grid that fails
...
| Whether tier-2 absence resets a counter | A seat is drawn into the top tier by a seed it does not control. Penalising it like a missed home ceremony may be right; it has not been argued. |
| The fetch bound in Road B | A proposal that demands too many tier-1 blocks a seat does not hold is a denial vector wearing the clothes of a larger set. |
