...
*Stages 1 and 2 are built — see §10 for what changed on contact.*
...
| `net/handshake.py` | new: the challenge-response of §4.1, against `doc.public_keys()` |
| `net/peer.py` | bounded accept and a worker pool, per-address connection cap, idle and partial-frame deadlines, the penalty box, meter before decode |
...
| 3 | the `hello` handshake; peers metered under authenticated names | not started — ends escalation and squatting together |
