...
| 7 | chunked snapshot transfer, so a state larger than a frame can sync | not started — the format already supports it; `_serve_snapshot` refuses rather than chunking |
...
| ~~The roll cannot be built from one view~~ | **Fixed in part nine, as sketched here.** The block at epoch e carries the certificate of e-1, and `AttendanceRoll.from_cert` derives attendance from the seats that signed it — so a seat that said nothing no longer earns attendance, and there is nothing left for seven views to disagree about. Two things the sketch did not anticipate: the certificate has to carry apprentices' *shadow* attestations, or an apprentice never appears in a roll and is never promoted; and the roll must be derived from the certificate **in the block**, not from the one this node assembled, because certs assembled after agreement differ per seat — checking the leader's against your own stalls the network within six epochs. Residual: a leader may omit attestations it saw, under-crediting seats by one epoch of streak. |
...
| ~~The lazy stamper, again~~ | **Half closed in part nine.** `lazy` was in the fault table because the design named it — the network path had one behaviour branch, `silent`, so nothing could produce the fault, let alone catch it. `Seat` now implements it, and `Seat.catch_lazy` proves it: an attestation over a block this seat found invalid is signed evidence its author did not check. What is still open is *acting* on the report — `GridRegister.apply` takes a `faulted` set and the network path passes none, because agreeing about a fault means carrying it in the block, which is a format change. And the honest limit: laziness is only catchable when there is something to catch. |
