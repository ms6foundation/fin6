...
| Viewing keys and disclosure | **Two of the four bounds are built; the other two are named.** The honest frame first: revocation is not available, because bytes cannot be un-given — an auditor keeps whatever it can already decrypt. What is available is bounds set in advance.

*Built, and neither touches the chain.* **Diversified addresses** — all three derivations now take an index, so one seed yields as many addresses as you like and a viewing key is scoped to *its* address; route a counterparty or a period through its own and the disclosure is that relationship rather than a life. Index 0 is byte-identical to the single key these functions used to return, so nothing existing moves. Change goes back to the address that paid, because sending it to index 0 would defeat the scoping quietly. **Per-output keys** — `disclosure_key` hands over the symmetric key for one sealed opening, derived from that output's own ephemeral key with its commitment hashed in, so it opens exactly one note and not the next payment to the same address. Self-proving: `open_disclosed` refuses unless the reconstructed note reproduces the commitment on chain, so an auditor is not taking the holder's word for an amount.

*Named, not built.* **Era-derived viewing keys** would fix the part diversifiers only mitigate — a key you hand over today still opens payments made to that address tomorrow — and it is a change to the address format, so it belongs before genesis. The construction is standard (rotating and diversified keys, non-hardened derivation) and the caveat is real: tweaking X25519 public keys needs care about scalar clamping, so the clean version does the point arithmetic in Ed25519 form and wants a reviewer. **Disclosure by proof** is the only truly revocable form — the auditor gets statements and proofs rather than keys, so revocation is simply ceasing to answer — and it costs a proof per note.

And the question underneath all of it, which is not technical: an auditor entitled to **completeness** ("you showed me everything") needs more than any of these. All four give *authenticity*. Completeness needs the detection secret for a bounded range, or the per-block output total the header already commits as `utxo_count`. A regulator's answer is usually completeness. |
| ~~Multi-note spends~~ **Built** | `Wallet.send` spends as many notes as a payment needs, and `Wallet.select` picks them from **one partition**: a note is spendable in exactly one grid, so a transaction is too, and one spending across two would have no grid able to include it (review B3). When no single grid holds enough, `select` says so and names the remedy; `Wallet.consolidate` is that remedy, gathering one grid's notes into one note in that grid. Change is steered back into the partition it came from with `note_in_partition` — the partition is a hash of the note and `rho` is randomness we draw anyway — because otherwise a balance scatters across grids one payment at a time. Selection is largest-first, which is what the old comment about proof size always meant; the sort had been the other way round. |
